...
os.environ['TORCH_HOME']='attack_benchmark'
...
def test(args):
    ...
    if args.dataset_name == "imagenet":
        ...
        test_transform = transforms.Compose([
            transforms.Resize(size, interpolation=3 if args.interpolation == 'bicubic' else 2),
            transforms.CenterCrop(args.input_size),
            transforms.ToTensor()
        ])
        print('test transforms: ', test_transform)
    
        imagenet = Imagenet('data/imagenet_val.txt', args.data_dir,
                                data_start=0,data_end=-1,transform=test_transform)
        test_loader = DataLoader(imagenet, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True, drop_last=False)
        test_loader.name = "imagenet"
        test_loader.batch = args.batch_size
    ...
    if args.dataset_name == "imagenet":
        ...
        for i, (image,labels,target_labels) in enumerate(test_loader, 1):
            ...
            if i ==1:
                ...
                load_path = os.path.join('test_out', filename)
