...
class APGD(Attack):
    ...
    """
    def __init__(self, model,dataset_name,device, norm='Linf', eps=8/255, steps=100, n_restarts=1, 
                 seed=0, loss='ce', eot_iter=1, rho=.75, verbose=False):
        super().__init__("APGD", model, dataset_name)
        self.eps = eps
        self.steps = steps
        self.norm = norm
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                logits = self.model(x_adv1) # 1 forward pass (eot_iter = 1)
...
class APGDT(Attack):
    ...
    def __init__(self, model, dataset_name, device, norm='Linf', eps=8/255, steps=100, n_restarts=1,
        ...
        super().__init__("APGDT", model,dataset_name)
        self.eps = eps
        self.steps = steps
        self.norm = norm
        self.n_restarts = n_restarts
