...
class FAB(Attack):
    ...
    """
    def __init__(self, model, dataset_name, norm='Linf', eps=None, steps=100, n_restarts=1,
                 alpha_max=0.1, eta=1.05, beta=0.9, verbose=False, seed=0,
                 targeted=False, device=None):
        super().__init__("FAB", model,dataset_name)
    ...
    def get_diff_logits_grads_batch(self, imgs, la):
        ...
        with torch.enable_grad():
            im1 = (im - self.mean_torch) / self.std_torch
            y = self.model(im1)
            ...
            if not self.targeted:
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        adv_curr1 = (adv_curr - self.mean_torch) / self.std_torch
                        acc_curr = self.model(adv_curr1).max(1)[1] == y_to_fool
                        if self.norm == 'Linf':
