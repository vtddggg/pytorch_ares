...
class Normalizer(object):
    ...
    def l1_normalize(perturb):
        l1n = torch.max(Normalizer.l1_norm(perturb), torch.ones(
            perturb.size(0), device="cuda:0") * 0.001)
    ...
    def l2_normalize(perturb):
        l2n = torch.max(Normalizer.l2_norm(perturb), torch.ones(
            perturb.size(0), device="cuda:0") * 0.001)
        perturb = perturb / l2n.view(-1, 1, 1, 1)
        return perturb

    @staticmethod
    def normalize(perturb, p):
        normalizers = {0: Normalizer.l0_normalize, 1: Normalizer.l1_normalize,
                       2: Normalizer.l2_normalize, np.inf: Normalizer.linf_normalize}
...
class Bounder(object):
    ...
    def l0_bound(perturb, epsilon):
        ...
        target_size = (perturb.size(0), ) + (1,) * \
            (len(perturb.size()) - len(thresholds.size()))
    ...
    def l0_bound_sparse(perturb, epsilon, x_victim):
        ...
        perturb_permuted_numpy = perturb.permute(
    ...
    def l2_bound(perturb, epsilon):
        ...
        return perturb * multiplier.view(-1, 1, 1, 1)

    @staticmethod
    def bound(perturb, epsilon, p):
        bounders = {0: Bounder.l0_bound, 1: Bounder.l1_bound,
                    2: Bounder.l2_bound, np.inf: Bounder.linf_bound}
